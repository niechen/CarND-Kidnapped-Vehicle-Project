...
inline double gaussian_prob(double obs_x, double obs_y, double mu_x, double mu_y, double sig_x, double sig_y) {
  double gauss_norm = (1.0/(2.0 * M_PI * sig_x * sig_y));
  double exponent = pow((obs_x - mu_x), 2.0)/(2.0 * pow(sig_x, 2.0)) + pow((obs_y - mu_y), 2.0)/(2.0 * pow(sig_y, 2.0));
//  std::cout << obs_x << ", " << obs_y << ", " << mu_x << ", " << mu_y << ", " << sig_x << ", " << sig_y  << std::endl;
